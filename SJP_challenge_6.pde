//2 libraries: sound and tank sprites 
//press keyboard makes sounds for each key 
...
StopWatch timer;
float tankSpeed = 0, tankMaxSpeed = 60, tankMaxRevSpeed = -10;
float tankHeading, tankRotateRate = 1.0f;
float tankAccelRate = 10, tankDecelRate = 15;
...
void setup() {
  ...
  for (int y = 10; y < width; y += 20) {
    for (int x = 10; x < width; x += 20) {
      ...
      if (b == 255) {
        ...
        s = new Sprite(this, "large.png", 50);
        s.setXY(x, y);
        targets.add(s);
      }
      else if (g == 255) {
  ...
  tShell = new Sprite(this, "pancake.png", 20);
  tShell.setFrame(0);
